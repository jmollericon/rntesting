...
export const Input = ({ label, error, ...props }: any) => {
  const containerStyles: Array<any> = [styles.inputContainer];
  if (error) {
...
export const Button = ({ text, onPress, ...props }: any) => {
  return (
...
export const ErrorText = ({ messages = [] }: any) => {
  const displayMessages = messages.filter((msg: any) => msg !== undefined);
